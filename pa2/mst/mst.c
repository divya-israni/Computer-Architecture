    ...
    for (unsigned iter=0; iter<graphNodeCount-1; iter++) {
        ...
        for (graphNode_t source=0; source<graphNodeCount; source++) {
            if (parents[source]!=-1) { // if already visited
                AdjacencyListNode* current = adjacencyList[source].next; //idk if the next should be there
                while(current!=NULL){
                    //graphNode_t dest = current->graphNode;
                    //double weight = current->weight;
                    if(parents[current->graphNode]==-1 && current->weight<minWeight){
